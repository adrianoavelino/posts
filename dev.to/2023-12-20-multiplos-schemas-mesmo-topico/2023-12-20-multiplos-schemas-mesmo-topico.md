...
Com isso podemos validar os subjects na [interface gráfica do landoop](http://localhost:3030/schema-registry-ui/#/cluster/fast-data-dev) e pesquisar pela palavra `transacoes` ou através do **curl** com o seguinte comando:
```bash
...
Também podemos visualizar os eventos recebidos no tópico Kafka usando a [interface gráfica do landoop](http://localhost:3030/kafka-topics-ui/#/cluster/fast-data-dev/topic/n/transacoes/) ou através de um consumer na linha de comando, num outro terminal:
...
Ao finalizar o passo a passo é possível registrar dois schemas no mesmo tópico. Para registrar os schemas utilizamos um `kafka-avro-console-producer` para armazenar os schemas de forma automática, também, definimos a estratégia de nome de subject com `TopicRecordNameStrategy` para não sofrer com problemas de conflito de subjects no mesmo tópico.
